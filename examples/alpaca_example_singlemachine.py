...
def main():
    ...
    scheduler = CosineScheduler(
        optimizer, warmup_steps=Config.warmup_steps, total_steps=total_steps
    )
        ...
        for _batch_idx, batch in enumerate(loader):
            ...
            if accum_steps % Config.grad_accum_steps == 0:
                ...
                if global_step % 10 == 0:
                    ...
                    print(
                        f"Epoch {epoch} | Step {global_step} | Loss {avg_loss:.4f} | LR {current_lr:.6f}"
                    )
    ...
    test_prompt = (
        ...
        + Config.instruction_template.format(instruction="Write a haiku about GPUs.")
        + "<|assistant|>\n"
    )
